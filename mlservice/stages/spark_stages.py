...
class Quantize(Stage, Fitable):
    ...
    @validator('input')
    def input_should_be_one_field(cls, v, values):
    ...
    def transform(self, coldata):
        initColumns = coldata.df.columns
        assert (isinstance(coldata, SparkColdata))
        ...
        if not self.splits is None:
            ...
            input = spark_encode_colnames([input])[0]
            output = spark_encode_colnames([self.output])[0]

            from pyspark.ml.feature import Bucketizer
            bucketizer = Bucketizer(splits=[float('-Inf')] + list(self.splits) + [float('Inf')], inputCol=input,
                                    outputCol=output)
            newdf = bucketizer.transform(coldata.df)
            newdf = newdf.select(initColumns + [output])
            coldata.setdf(newdf)
        else:
            input_col = spark_removeDotFromColnames([input])[0],
            label_col = spark_removeDotFromColnames([self.label])[0],
            output_col = spark_removeDotFromColnames([self.output])[0]
            ...
            newdf = assembler.transform(coldata.df)

            newdf = self.model.transform(newdf)
            newdf = newdf.select(initColumns + [output_col])
            coldata.setdf(newdf)
    ...
    def fit_(self, coldata_subset):
        ...
        if self.method == 'decision_tree':
            ...
            if isinstance(input, str):
                self.splits = Quantize.quantize_using_decision_tree_1d(coldata_subset,
                                                                       spark_removeDotFromColnames([input])[0],
                                                                       spark_removeDotFromColnames([self.label])[0],
                                                                       self.len)
                self.model = None
                self.len = len(self.splits) + 1  # It seems that sometimes it change
            else:
                self.model = Quantize.quantize_using_decision_tree(coldata_subset,
                                                                   spark_removeDotFromColnames([input])[0],
                                                                   spark_removeDotFromColnames([self.label])[0],
                                                                   spark_removeDotFromColnames([self.output])[0],
                                                                   self.len)
                self.splits = None
                self.len = Quantize.getSparkDecisionTreeNumLeaves(self.model)
        else:
    ...
    def model_to_json(self):
        if self.splits is None:
            raise Exception(
                'self.splits is None (you may have used more than one input field which is not supported yet).')
    ...
    @staticmethod
    def quantize_using_decision_tree(coldata_subset, input_col, label_col, output_col, num_splits):
        from pyspark.ml.feature import VectorAssembler

        assembler = VectorAssembler(
            inputCols=[input_col],
            outputCol="spark_tmp_features")
        newdf = assembler.transform(coldata_subset.df)
        newdf = newdf.withColumn(label_col, newdf[label_col].cast(DoubleType()))

        from pyspark.ml.regression import DecisionTreeRegressor
        # For some reason DecisionTreeClassifier did not work with just one feature
        clf = DecisionTreeRegressor(featuresCol='spark_tmp_features',
                                    labelCol=label_col, maxDepth=30,
                                    maxBins=num_splits,
                                    leafCol=output_col)  # 30 is the highest depth allowed in pyspark!
        model = clf.fit(newdf)

        return model

    @staticmethod
    def quantize_using_decision_tree_1d(coldata_subset, input_col, label_col, num_splits):
        from pyspark.ml.feature import VectorAssembler
        ...
        clf = DecisionTreeRegressor(featuresCol='spark_tmp_features',
                                    labelCol=label_col, maxDepth=30,
                                    maxBins=num_splits)  # 30 is the highest depth allowed in pyspark!
        model = clf.fit(newdf)
        allTresholds = Quantize.getSparkDecisionTreeThresholds_1d(model)

        return allTresholds

    @staticmethod
    def getSparkDecisionTreeThresholds_1d(model):
        def traverse(node, res):
            if not 'split' in dir(node):  # LeafNode
                return []

            traverse(node.leftChild(), res)
            if not node.split().featureIndex() == 0:
                raise Exception('This method only works with decision trees using 1d features.')
            res.append(node.split().threshold())
            traverse(node.rightChild(), res)

        root = model._java_obj.rootNode()
        res = []
        traverse(root, res)
        return res

    @staticmethod
    def getSparkDecisionTreeNumLeaves(model):
        def traverse(node, res):
            print(res)
            if not 'split' in dir(node):  # LeafNode
                return res + 1

            res = traverse(node.leftChild(), res)
            #         if not node.split().featureIndex()==0:
            #             raise Exception('This method only works with decision trees using 1d features.')
            res = traverse(node.rightChild(), res)
            return res
